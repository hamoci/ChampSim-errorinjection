...
# GAP-suite traces are stored as e.g. "bc-12.trace.gz"; strip the compression
# suffix so the workload id reads as "bc-12". SPEC traces ("602.gcc_s-1850B")
# carry no such suffix and fall through to WORKLOAD_RE unchanged.
TRACE_SUFFIXES = (".trace.gz", ".trace.xz", ".champsimtrace.xz",
                  ".champsimtrace.gz", ".champsim.xz", ".trace", ".xz", ".gz")
...
def gmean(values, include_zeros=False):
    """Geometric mean.

    Default: skip None / non-positive values (legacy behaviour).

    include_zeros=True: count every non-None value, treating a 0 as a real
    data point. Because log(0) = -inf, any single 0 collapses the result to
    0.0 — this is intentional, so a config containing an incomplete/panicked
    run (recorded as IPC 0) shows a 0-height bar. Returns NaN when no value
    is present at all (so the bar is simply absent, not 0)."""
    if include_zeros:
        ...
        if any(v <= 0 for v in present):
            return 0.0
        return float(np.exp(np.mean(np.log(present))))
...
_XLSX_PATH = os.path.join(SCRIPT_DIR, "raw_data.xlsx")
_xlsx_cache = {}
