#!/usr/bin/env python3
"""
ChampSim GAP Benchmark Error Rate별 IPC 비교 스크립트 (논문 스타일)
동일 Capacity에서 1e-2 ~ 1e-9 까지의 Error Rate별 IPC 비교
GAP Benchmarks: bc, bfs, cc, pr, sssp
"""
...
def parse_results_directory(results_dir):
    ...
    for filename in os.listdir(results_dir):
        ...
        # GAP benchmark인지 확인
        is_gap = any(f'-{bench}-' in filename or f'_{bench}-' in filename for bench in gap_benchmarks)
        if not is_gap:
            continue

        file_path = os.path.join(results_dir, filename)

        # 파일명 파싱: champsim_{pagesize}_error_{capacity}_{error_rate}_{workload}.trace.gz.txt
        # 예: champsim_2mb_error_128gb_1e-7_bc-12.trace.gz.txt
        pattern = r'champsim_(\w+)_error_(\w+)_(1e-\d+)_(bc|bfs|cc|pr|sssp)-(\d+)\.trace\.gz\.txt'
        ...
        if match:
            ...
            bench_num = match.group(5)

            # 워크로드명: benchmark-number 형식
            workload = f"{benchmark}-{bench_num}"
...
def create_average_comparison_plot(data, page_size='2mb', capacity='128gb', output_dir='results'):
    """Average IPC 비교 그래프 생성 (GAP Benchmarks)"""
    ...
    for er in available_error_rates:
        ...
        if valid_ipcs:
            ...
            ipc_values_gap.append(avg_ipc)
        else:
            ipc_values_gap.append(None)

    # X축 값을 실제 error rate 숫자로 변환
    x_values = [float(er.replace('e-', 'e-')) for er in available_error_rates]

    # GAP 그래프 (파란색 실선)
    valid_points = [(x, y) for x, y in zip(x_values, ipc_values_gap) if y is not None]
    if valid_points:
        x_plot, y_plot = zip(*valid_points)
        ax.plot(x_plot, y_plot, 'o-', color='#2E86DE', linewidth=4,
               markersize=12, label=f'GAP {page_size.upper()}', zorder=3)
    ...
    if has_4kb:
        ...
        if valid_points_4kb:
            ...
            ax.plot(x_plot_4kb, y_plot_4kb, 'o--', color='#5F27CD', linewidth=4,
                   markersize=12, label=f'GAP {page_size_4kb.upper()}', zorder=3)
    ...
    # Y축 범위 자동 설정
    all_values = [v for v in ipc_values_gap if v is not None]
    if has_4kb:
    ...
    plt.tight_layout()
    output_file = f'/home/hamoci/Study/ChampSim/{output_dir}/gap_average_ipc_comparison_{page_size}_{capacity}.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
    print(f"GAP 평균 IPC 그래프 저장: {output_file}")

    # IPC 감소율 출력
    print(f"\n{'='*80}")
    print(f"GAP IPC 감소율 분석 - {page_size.upper()} {capacity.upper()}")
    print(f"{'='*80}")

    if ipc_values_gap[0] is not None:
        baseline_gap = ipc_values_gap[0]  # 1e-2
        print(f"\n[{page_size.upper()}] Baseline (1e-2): {baseline_gap:.4f}")
        print(f"{'Error Rate':<15} {'IPC':<12} {'감소율 (%)':<15} {'절대 감소':<15}")
        print("-" * 60)

        for i, er in enumerate(available_error_rates):
            if ipc_values_gap[i] is not None:
                ipc = ipc_values_gap[i]
                decrease_pct = ((baseline_gap - ipc) / baseline_gap) * 100
                abs_decrease = baseline_gap - ipc
...
def create_per_benchmark_plots(data, capacity='128gb', output_dir='results'):
    ...
    for bench_name in sorted(benchmarks.keys()):
        ...
        for idx, workload in enumerate(workloads):
            ...
            # 워크로드 이름 정리
            workload_label = workload.split('-')[1] if '-' in workload else workload
        ...
        plt.tight_layout()
        output_file = f'/home/hamoci/Study/ChampSim/{output_dir}/gap_{bench_name}_ipc_{capacity}.png'
        plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
        print(f"GAP {bench_name.upper()} 그래프 저장: {output_file}")
        plt.close()

def create_all_benchmarks_comparison(data, page_size='2mb', capacity='128gb', output_dir='results'):
    """모든 벤치마크의 평균 IPC를 하나의 그래프에 표시"""
    ...
    ax.set_title(f'GAP Benchmarks Average IPC - {page_size.upper()} ({capacity.upper()})',
    ...
    plt.tight_layout()
    output_file = f'/home/hamoci/Study/ChampSim/{output_dir}/gap_all_benchmarks_comparison_{page_size}_{capacity}.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
    print(f"GAP 전체 벤치마크 비교 그래프 저장: {output_file}")
    plt.close()
...
def print_summary_table(data, page_size='2mb', capacity='128gb'):
    ...
    print("\n" + "="*100)
    print(f"GAP BENCHMARK ERROR RATE COMPARISON - {page_size.upper()} Page Size, {capacity.upper()} Capacity")
    print("="*100)
...
def main():
    ...
    print("ChampSim GAP Benchmark Error Rate 비교 결과 파싱 중...")
    data = parse_results_directory(results_dir)

    if not data:
        print("파싱된 GAP 데이터가 없습니다.")
        return
    ...
    print("\n" + "="*80)
    print("완료! 모든 GAP 그래프가 results 폴더에 저장되었습니다.")
    print("="*80)
    print("\n생성된 파일:")
    print("  - gap_average_ipc_comparison_{pagesize}_{capacity}.png: GAP 평균 IPC 비교")
    print("  - gap_all_benchmarks_comparison_{pagesize}_{capacity}.png: 모든 GAP 벤치마크 비교")
    print("  - gap_{benchmark}_ipc_{capacity}.png: 각 벤치마크별 상세 비교 (bc, bfs, cc, pr, sssp)")
