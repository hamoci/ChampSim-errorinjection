...
def parse_category_directory(category_dir, category_name):
    """카테고리별 디렉토리에서 결과 파싱 (no_error_baseline용)"""
    data = {}
    ...
    for filename in os.listdir(category_dir):
        ...
        # 파일명 패턴 파싱 (no_error_baseline 형식)
        # Server: champsim_{pagesize}_{capacity}_{workload}.champsim.trace.gz.txt
        # GAP: champsim_{pagesize}_{capacity}_{workload}.trace.gz.txt
        # SPEC: champsim_{pagesize}_{capacity}_{workload}.txt

        # Server 패턴 (*.champsim.trace.gz.txt)
        pattern_server = r'champsim_(\w+)_(\w+)_(.+?)\.champsim\.trace\.gz\.txt'
        # GAP 패턴 (*.trace.gz.txt)
        pattern_gap = r'champsim_(\w+)_(\w+)_(.+?)\.trace\.gz\.txt'
        # SPEC 패턴 (*.txt)
        pattern_spec = r'champsim_(\w+)_(\w+)_(.+?)\.txt'
...
def print_summary(data):
    ...
    print("\n" + "="*120)
    print("ALL WORKLOADS MEMORY INTENSITY ANALYSIS (No-Error Baseline)")
    print("="*120)
...
def main():
    """메인 함수"""
    # 스크립트 위치 기준으로 상대 경로 설정 (stat_script 디렉토리에서 실행)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.join(script_dir, '..', 'results', 'no_error_baseline')
    output_dir = script_dir  # 결과를 stat_script 디렉토리에 저장

    # 카테고리별 디렉토리
    categories = {
        'SPEC': os.path.join(base_dir, 'spec'),
        'Server': os.path.join(base_dir, 'server'),
        'GAP': os.path.join(base_dir, 'gap')
    }

    print("ChampSim No-Error Baseline 워크로드 RBMPKI & IPC 분석 시작...")
    print("="*100)
    ...
    # CSV 파일 저장
    csv_file = os.path.join(output_dir, 'baseline_workloads_rbmpki_ipc.csv')
    ...
    # 카테고리별 평균 비교
    cat_comparison_file = os.path.join(output_dir, 'baseline_rbmpki_category_comparison.png')
    create_bar_chart_by_category(all_data, cat_comparison_file)

    # 전체 워크로드 순위 (RBMPKI 순)
    all_ranking_file = os.path.join(output_dir, 'baseline_rbmpki_all_ranking.png')
    create_combined_bar_chart_by_rbmpki(all_data, all_ranking_file)

    # 전체 워크로드 (이름순)
    all_by_name_file = os.path.join(output_dir, 'baseline_rbmpki_all_by_name.png')
